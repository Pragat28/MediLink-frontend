...
function MyAppointments() {
  ...
  // ✅ Use ref to track if this is the first fetch
  ...
  // ✅ Show unseen notifications on mount
  useEffect(() => {
  ...
  const fetchAppointments = async () => {
    ...
    try {
      ...
      // ✅ Only check for status changes after the first fetch
      ...
      // ✅ After first fetch, mark as done and save the map
  ...
  const submitRating = async (appointmentId) => {
    ...
    try {
      ...
      if (!err.response) {
        toast.error("Network error — could not submit rating.");
      } else {
    ...
    return list.map(app => (
      <div key={app._id} style={cardStyle}>
        ...
        <p style={{ color: "#64748b", margin: "0 0 10px" }}>
          ...
          {app.mode && <span> &nbsp;· {app.mode === "online" ? "🌐 Online" : "🏥 Offline"}</span>}
        </p>

        {/* ── ACCEPTED ── */}
        {type === "accepted" && (
          <>
            <p style={{ margin: "4px 0", fontSize: "14px" }}>📧 {app.doctor?.email}</p>
            <p style={{ margin: "4px 0", fontSize: "14px" }}>📞 {app.doctor?.phone}</p>
        ...
        {type === "pending" && (
          <div style={{
            ...
            <p style={{ margin: 0, color: "#d97706", fontWeight: "600" }}>⏳ Awaiting Doctor's Approval</p>
        ...
        {type === "rejected" && (
          <div style={{
            ...
            <p style={{ margin: 0, color: "#64748b", fontWeight: "600" }}>🚫 Appointment Rejected</p>
          ...
          <>
            {app.status === "completed" && (
              <div style={{
                ...
                <p style={{ margin: 0, color: "#16a34a", fontWeight: "600" }}>✅ Appointment Completed</p>
              </div>
            )}
            ...
            {app.status === "cancelled" && (
              <div style={{
                ...
                <p style={{ margin: 0, color: "#64748b", fontWeight: "600" }}>🚫 Appointment Cancelled</p>
                <p style={{ margin: "4px 0 0", fontSize: "13px", color: "#94a3b8" }}>
                  This appointment was cancelled. You may book a new one.
                </p>
              </div>
            )}
